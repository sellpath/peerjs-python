...
AMBIANIC_PNP_HOST = 'dev-peerjs.sellpath.ai'  # 'localhost'
AMBIANIC_PNP_PORT = 443  # 9779
AMBIANIC_PNP_SECURE = True  # False
DEFAULT_LOG_LEVEL = 'DEBUG'

# Set the logging level for the peerjs module
logging.getLogger('peerjs').setLevel(DEFAULT_LOG_LEVEL)
    ...
    while not _is_shutting_down:
        ...
        try:
            if not peer or peer.destroyed:
                logger.info(f'Peer destroyed. Will create a new peer. savedPeerId={savedPeerId}')
                peer = await pnp_service_connect()
            elif peer.open:
                # await join_peer_room(peer=peer)
                logger.info(f'Peer open savedPeerId={savedPeerId}')
            elif peer.disconnected:
                logger.info(f'Peer disconnected. Will try to reconnect. savedPeerId={savedPeerId}')
                await peer.reconnect()
            else:
                logger.info(f'Peer still establishing connection. {peer}   savedPeerId={savedPeerId}')
        except Exception as e:
            logger.exception(f'Error while trying to join local peer room. savedPeerId={savedPeerId}'
            ...
            if peer and not peer.destroyed:
                ...
                logger.info(f'Peer connection was corrupted. Detroying peer. savedPeerId={savedPeerId}')
                await peer.destroy()
                peer = None
...
#     peer = Peer(id="savedPeerId", peer_options=options)
#     await peer.start()
#     print('peer activated')
#     _setPnPServiceConnectionHandlers(peer)  # You might need to comment out this line depending on your needs
